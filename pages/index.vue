<template>
  ...
  onMounted(() => {
    // Проверяем, что Telegram Web Apps SDK доступен
    if (window.Telegram && window.Telegram.WebApp) {
      ...
      if (user) {
        ...
        // Пример: получение themeParams
        const themeParams = window.Telegram.WebApp.themeParams;
        console.log("Theme Params:", themeParams);
        //Пример: установка бэкграунда
        if (themeParams.bg_color) {
            document.body.style.backgroundColor = themeParams.bg_color;
        }
  
        // window.Telegram.WebApp.sendData(JSON.stringify({ userId: user.id, items: items.value }));
      } else {
